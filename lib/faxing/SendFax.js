...
function sendTelnyxFax(calledNumber, callingNumber, pdfFile) {
	...
	request(options, function (error, response, body) {
	  ...
	  var faxDirection = jsonBody.data.direction
	  var telnyxfaxId = jsonBody.data.id
	  var status = jsonBody.data.status
	  var numberFrom = jsonBody.data.from
	  var numberTo = jsonBody.data.to
	  var ataFaxId = path.parse(pdfFile).name
	  logFaxEntry.create(telnyxfaxId,ataFaxId,null,null,numberFrom,numberTo,faxDirection,null,status,(faxLogRec) => {console.log(faxLogRec)});
		//console.log(body)
	});
