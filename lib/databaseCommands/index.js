...
function addOrUpdatePhoneNumbers(device) {
	var sqlStatements = []
	if (device.line1) {
		sqlStatements.push({clientid: device.clientid, phonenumber: device.line1, deviceid: device.id, line: 1})
	} else if (device.line2) {
		sqlStatements.push({clientid: device.clientid, phonenumber: device.line2, deviceid: device.id, line: 2})
	}
	console.log('Log attempt 1')
	console.log(sqlStatements)
	console.log('Log attempt 2')
	sqlStatements.map(obj => console.log(obj))
		...
		return new Promise(function (resolve,reject) {
			switch (recordType) {
				case "Clients":
					...
					break;
				case "Devices":
					addOrUpdatePhoneNumbers(statement)
