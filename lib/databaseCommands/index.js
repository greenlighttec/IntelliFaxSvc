...
function addOrUpdatePhoneNumbers(id,device) {
	...
	PhoneNumbers.bulkCreate(sqlStatements, {updateOnDuplicate: ['phonenumber'] })
...
module.exports = {
	...
	lookupAccountIdByFaxNo: function (faxNo) {
		let phoneNumberAttributes = ['phonenumber']
		let deviceAttributes = ['username']
		return new Promise(function (resolve,reject) {
			var statement = {
				include: [{
					model: PhoneNumbers,
					attributes: phoneNumberAttributes,
					where: {phonenumber: faxNo}
				}],
				attributes: deviceAttributes
			}
			Devices.findOne(statement)
