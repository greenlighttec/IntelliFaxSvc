...
module.exports = {
	...
	authenticateAccount: function(macaddr, password, cb) {
        ...
        authenticateUser: function(userid, password, cb) {
				...
				users.updateRecordByWhereStatement({id: userid}, {session: sessionId}, (updatedUser) => {
					cb(sessionId)
				})
			}
                        else { cb(false); }
	...
	findClientIdBySessionId: function(sessionid, cb) {
		users.findAllByStatement({attributes: ['clientid','session'], where: {session: sessionid}}, (user) => {
			if (user) {
				cb(user)
			} else {cb(false)}
		})
	...
	userLogout: function(sessionid, cb) {
		users.updateRecordByWhereStatement({session: sessionid}, {session: null}, (updatedUser) => cb(updatedUser))
	}
